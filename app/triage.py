...
URGENT_PATTERNS = [
    ...
    (r"(至急|急ぎ|すぐ|早く)", "急ぎの気配"),
    (r"不在着信", "折り返し待ちの電話"),   # v222: 電話をかけてくる=急ぎのことが多い
...
def ai_classify(text: str, rank: str) -> tuple[str, str] | None:
    ...
    if not config.TRIAGE_AI or not config.ANTHROPIC_API_KEY:
        return None
    # v158: 一般モード(CHOUBA_MODE=general)は客-ホステスでなく1対1の人間関係として仕分ける
    if config.MODE == "general":
        _who = ("あなたは多忙な人の受信LINEを2分類する仕分け係。\n"
                "urgent=今すぐ返した方がよい(会う約束/日程/仕事の依頼・確認/金銭/急ぎ、"
                "相手が返事を待つ問いかけ、不満・不機嫌など感情が強い)。\n")
    else:
        _who = ("あなたは銀座クラブのホステスの受信LINEを2分類する仕分け係。\n"
