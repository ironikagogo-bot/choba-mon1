...
【既定トーン＝カジュアル敬語寄り（実例がある相手は実例優先）】
- 初対面・実例の無い相手には「です・ます」を軽く残した砕けすぎない文。仲間内の実例がある相手はそのノリに合わせる。
- タメ口ベース。短く、崩して、句読点は少なめ。きれいな作文にしない。
- 接客・営業の定型句を使わない。禁止例:「お待ちしております」「ご用意しておきます」「楽しみにしております」「〜させていただきます」の連発、「嬉しいです」の多用、過剰な持ち上げ、堅い時候の挨拶。
...
def _template_drafts(contact: dict, text: str, reason: str) -> list[dict]:
    ...
    if "日程" in reason or "来店" in reason or "席" in reason:
        return [
            {"tone": "軽く", "text": "いいね、会お〜。今週なら火曜か木曜が動きやすいけどどう？"},
            {"tone": "最小", "text": "りょ！日にち決まったら教えて〜"},
        ]
    return [
...
def generate(message_id: int) -> list[dict]:
    ...
    _kind = (contact.get("kind") or "").strip()
    if _kind == "staff":
        mode_prompt += ("\n【その他モード】分類していない相手(手続き・業者・付き合いの浅い知人など)。"
                        "丁寧すぎない敬語で簡潔に、要件を明確に。約束・金額・日時は断定せず確認で返す。")
    elif _kind == "peer":
        mode_prompt += ("\n【家族・友人モード】相手は家族や親しい友人。挨拶や社交辞令は不要、飾らない口調で短く。"
                        "絵文字や『！』は本人の実例にある範囲だけ。用件＋ひとこと気遣い程度で十分。")
    elif _kind == "excolleague":
        _st = (contact.get("stand") or "").strip()
        _tone = {"senior": "相手は目上(元上司・先輩など)。カジュアル敬語でも礼は守る。",
                 "junior": "相手は後輩。フランクでよいが雑にしない。"}.get(_st, "相手は元同業の仲間。対等なカジュアル敬語で。")
        mode_prompt += ("\n【元同業モード】仕事のつながりだが今は堅い間柄ではない。"
                        "カジュアルな敬語(です・ます軽め、堅苦しい定型句なし)で、近況や実務をテンポよく。" + _tone)
    elif _kind == "customer":
        mode_prompt += ("\n【柔術仲間モード】道場・練習仲間。ノリよく短く、体育会系の軽さでよい。"
                        "練習日程・スパーの約束は日時をはっきり確認。ケガや体調の話には一言気遣いを。")
